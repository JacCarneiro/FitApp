...
class _SchedulerPageState extends State<SchedulerPage> {
  ...
  Future<void> _selectTime(BuildContext context) async {
    ...
    if (pickedTime != null) {
      ...
      if (_isTimeSlotTaken(userProvider, schedule)) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Já existe uma consulta agendada para este horário.'),
          ),
        );
      } else {
  ...
  bool _isTimeSlotTaken(UserProvider userProvider, Schedule newSchedule) {
    // Verifica se existe um agendamento para o mesmo dia e horário
    final schedules = userProvider.activeUser?.schedules ?? [];
    return schedules.any((schedule) =>
        schedule.date.isAtSameMomentAs(newSchedule.date) &&
